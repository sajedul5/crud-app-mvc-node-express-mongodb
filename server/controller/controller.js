...
exports.create = (req, res) => {
    ...
    user
        ...
        .catch(err => {
            res.status(500).send({
                message: err.message || 'Some error occurred while creating a create oparation!'
            });
        });
...
exports.find = (req, res) => {
    if(req.query.id){
        ...
        Userdb.findById(id)
         ...
         .catch(err => {
             res.status(500).send({
                 message: 'Error retriveing user with id' +id
             })
         });
    }else{
    Userdb.find()
...
exports.update = (req, res) => {
    ...
    Userdb.findByIdAndUpdate(id, req.body, {useFindAndModify:false})
     .then(data => {
         if(!data){
            res.status(400).send({
                message: `Connot update user with ${id}. Maybe user not found!`
            });
         }else{
             res.send(data);
         }
